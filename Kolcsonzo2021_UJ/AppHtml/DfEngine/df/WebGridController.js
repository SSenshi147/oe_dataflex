...
selectRow : function(sGotoRow, iRow, fOptHandler, tOptSelectRowData){
    ...
    df.WebGridController.base.selectRow.call(this, sGotoRow, iRow, function(bSuccess, bOverridden){
        ...
        if(bSuccess && this.bNewRow){
            ...
            if(iNewRow >= 0){
                //  If appending a new row from a new row (press down in new row) without changes it can be that the current row is still the previous new row, we decrement the current row to prevent that from causing issues
                if(oM.iCurrentRow == iNewRow){
                    oM.iCurrentRow--;
                }
...
nextCol : function(){
    ...
    if(!oL._aColumns[iCol].isEnabled() || (oL._aColumns[iCol].validate && oL._aColumns[iCol].validate())){
        ...
        }while(!oL._aColumns[iCol].isEnabled() || !oL._aColumns[iCol].pbRender || !oL._aColumns[iCol].pbVisible);
