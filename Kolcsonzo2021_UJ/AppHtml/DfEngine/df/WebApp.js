...
df.WebApp = function WebApp(sOptWebService){
    ...
    df.log("Copyright © Data Access Corporation 2021");
...
sendCall : function(oCall){
    var aViews, aActions, aAttachments, oAction, oData, i, oRequest;
    ...
    function handleCall(oEvent, bSuccess){
        ...
        try{
            if(bSuccess){
                //  Parse response
                tData = oEvent.oSource.getResponseValue();

                if(tData && tData.r && tData.a && tData.r.Header && tData.r.asReturnValues){
                    tResponse = tData.r;
