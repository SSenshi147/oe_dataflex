...
df.WebBaseSelectionForm = function WebBaseSelectionForm(sName, oParent) {
    ...
    this.prop(df.tBool, "pbAllowTyping", true);
...
df.defineClass("df.WebBaseSelectionForm", "df.WebForm", {
    ...
    afterRender: function () {
        ...
        this._eControl = this._hInputField = df.dom.query(this._hInputBox, "input");
    ...
    */
    ...
    set_pbEnabled: function (bVal) {
        df.WebBaseSelectionForm.base.set_pbEnabled.call(this, bVal);
        this.set_pbAllowTyping(bVal);
    },

    set_pbAllowTyping: function (bVal) {
        if (this._hInputField) this._hInputField.disabled = !bVal;
    },
    ...
    encode: function (sValue) {
        ...
        var that = this;
        Object.keys(htmlEscapes).forEach(function (sEscape) {
            sValue = that._replaceAll(sValue, sEscape, htmlEscapes[sEscape]);
        });
    ...
    */
    suggestSelect: function () {
        ...
        if (this._oSelectedSuggestion && this._oSelectedSuggestion.control) {
            switch (this._oSelectedSuggestion.sRowId) {
                case "more":
                    ...
                    return;
                case "create":
                    if (this.pbAllowCreate && sValue.length >= this.piMinimumItemLength) {
        ...
        else if (this.pbAllowCreate == 1 && sValue.length >= this.piMinimumItemLength) {
            this.serverAction("CreateNewByValue", [sValue], null, null);
        }
    ...
    */
    suggestHandle: function (sVal, bLast) {
        ...
        this.updateTypeVal();
        if (this._hInputField) sCurVal = this.getSearchValue();
    ...
    */
    AddControlSuggestions: function (aList) {
        ...
        if (bShow && this.pbAllowCreate && sValue.length > 0) aList.push({ control: true, sRowId: "create", aValues: ["(" + this.psCreateNewItemText + (sValue.length > 0 ? " \"" + df.dom.encodeAttr(this.encode(sValue)) + "\"" : "") + ")"], sCssClassName: "Sugg_ControlSuggestion" });
    ...
    suggestRender: function (sSearch) {
        ...
        function renderSuggestion(iIndex, oSuggestion, aHtml) {
            ...
            for (x = 0; x < oSuggestion.aValues.length; x++) {
                sVal = that.encode(oSuggestion.aValues[x]);
    ...
    */
    suggestDoLoad: function (sVal) {
        if (!this.pbAllData || this._aSuggestRows.length == 0) {
        ...
        else this.suggestHightlight(this._oSelectedSuggestion);
    },
