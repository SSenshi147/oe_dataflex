...
df.WebApp = function WebApp(sOptWebService){
    ...
    df.log("Copyright(c) Data Access Corporation 2018");
...
initialize : function(bLoadDefault){
    ...
    try{
        if(this.checkBrowser()){
            this.serverAction("LoadWebApp", [ df.fromBool(!!bLoadDefault), df.sys.locale.language(), df.sys.locale.timezoneOffset() ], null, function(oEvent){
...
checkBrowser : function(){
    if(df.sys.isIE && df.sys.iVersion < 8){
        alert("Unfortunately the browser you are using is not supported by this framework. Please upgrade to a more modern browser.\n\r\n\rThe minimal version required is Internet Explorer 11. We advise to use the latest version of Google Chrome or Mozilla FireFox.");
        return false;
    }   
        ...
        try{
            ...
            this.serverActionEx({
                ...
                fHandler : function(oEv){
                    ...
                    oView = this[sView];
                    if(oView && oView instanceof df.WebView){
...
renderView : function(sView){
    this.ready(function(){
        ...
        if(!oView._bRendered){
            if(oView instanceof df.WebWindow){
...
showYesNo : function(sMessage, fHandler, oOptEnv){
    this.showMsgBox(sMessage, oWebApp.getTrans("confirm"), "WebMsgBoxConfirm", [ oWebApp.getTrans("yes"), oWebApp.getTrans("no")], 0, function(iBtn, oDialog){
        try{
            if(fHandler){
...
get_psCurrentView : function(){
    return (this._oCurrentWindow && this._oCurrentWindow._sName) || (this._oCurrentView && this._oCurrentView._sName) || "";
}
