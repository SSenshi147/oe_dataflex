...
afterRender : function(){
    ...
    //  Attach listener
    if(this._eControl){
        df.dom.on("keydown", this._eControl, this.filterPrep, this);
...
set_peDataType : function(iVal, bSvr){
    ...
    if(this._eControl){
        ...
        if(this._bFilters){
            df.dom.off("keypress", this._eControl, this.filterDate);
            df.dom.off("keypress", this._eControl, this.filterNumeric);
            df.dom.off("keyup", this._eControl, this.enhanceNumeric);
            
            if(iVal === df.ciTypeBCD){
                df.dom.on("keypress", this._eControl, this.filterNumeric, this);
                df.dom.on("keyup", this._eControl, this.enhanceNumeric, this);
            }else if(iVal === df.ciTypeDate || iVal === df.ciTypeDateTime || iVal === df.ciTypeTime){
                df.dom.on("keypress", this._eControl, this.filterDate, this);
            }
        }
...
typeToDisplay : function(tVal){
    ...
    if(!this._bHasFocus && this.psMask && this.peDataType !== df.ciTypeText){    //  If the field doesn't have the focus we need to apply a mask
        if(this.peDataType === df.ciTypeDate || this.peDataType === df.ciTypeDateTime || this.peDataType === df.ciTypeTime){ // Date mask
        ...
        if(tVal !== ""){    // Leave blank value alone
            if(this.peDataType === df.ciTypeBCD){   //  Plain number
                ...
                sVal = (tVal && df.sys.data.dateToString(tVal, this.timeFormat(), this.dateSep(), this.timeSep())) || "";
            }
        }
    }
...
initMask : function(){
    ...
    if(this._bMasks && this._eControl){
        ...
        this._aMaskChars = null;
        if(this._bFilters){
            df.dom.off("keypress", this._eControl, this.filterWinMask, this);
            
            df.dom.off("keyup", this._eControl, this.correctWinMask, this);
            df.dom.off("blur", this._eControl, this.correctWinMask, this);
            df.dom.off("cut", this._eControl, this.onCutPasteWinMask, this);
            df.dom.off("paste", this._eControl, this.onCutPasteWinMask, this);
        }
        ...
        if(this.peDataType === df.ciTypeText && this.psMask){
            ...
            //  Attach listeners
            if(this._bFilters){
                df.dom.on("keypress", this._eControl, this.filterWinMask, this);
                
                df.dom.on("keyup", this._eControl, this.correctWinMask, this);
                df.dom.on("cut", this._eControl, this.onCutPasteWinMask, this);
                df.dom.on("paste", this._eControl, this.onCutPasteWinMask, this);
            }
        }
    }
