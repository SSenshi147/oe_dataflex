...
df.WebListController = function WebListController(oList, oModel){
    this.oL = oList;
    this.oM = oModel;
...
df.defineClass("df.WebListController", {
...
fireRowClick : function(sRowId, iCol){
    var bHandled;
    ...
    bHandled = this.oL.fire("OnRowClick", [ sRowId ], function(oEv){
...
@param  iCol    Column index.
*/
headDblClick : function(oEv, iCol){
    return false;
},
...
@private
*/
keyPress : function(oEv){
    var oL = this.oL, sChar, iKey, iChar;
    ...
    //  Auto search
    if(oL.pbAutoSearch && oL.piSortColumn >= 0 && !this.bNoSearch && oL._bHasFocus){
        iKey = oEv.getKeyCode();
...
handleData : function(aRows, sType, sStartRowId, bFirst, bLast){
    this.oM.handleData(aRows, sType, sStartRowId, bFirst, bLast);
...
selectRow : function(sGotoRow, iRow, fOptHandler, tOptSelectRowData){
    ...
    if(bContinue){
        ...
        if(!oL.pbOfflineEditing){
            ...
            oL.serverAction("ChangeCurrentRow", [ sGotoRow, sTargetRowId ], (tSelectRowData && oL.serializeVT([ tSelectRowData ])) || null);
        }else{
...
triggerCacheUpdate : function(iTopRow, iLastRow){
    var oM = this.oM;
...
processDataSet : function(eOperation){
    ...
    oL.serverAction("HandleProcessDataSet", [ eOperation, oM.iCurrentRow ], oL.serializeVT(oM.aData));
},
