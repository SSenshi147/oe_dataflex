...
df.dragdrop = (function dragdrop(){
    ...
    df.dom.ready(function(){
        df.dom.on("dragenter", document, onWindowEnter, this);
        df.dom.on("dragover", document, onWindowOver, this);
        df.dom.on("drop", document, onWindowDrop, this);
        df.dom.on("dragleave", document, onWindowLeave, this);
    });
...
df.uploadFiles = function uploadFile(oOpts){
    var i, fEmpty, aFiles, oWO, aRows = [], oCurrent, bCanceled, bUploading = true, oCurRec;
    ...
    */
    ...
    if(aFiles && aFiles.length > 0){
        for(i = 0; i < aFiles.length; i++){
            ...
            //  Guess extension file based on mime type (if name not availalble)
            if(!oCurrent.oFile.name){
                if(oCurrent.oFile.type){
                    if(oCurrent.oFile.type.indexOf("/") > 0){
                        oCurrent.oFile.name = "Unknown." + oCurrent.oFile.type.substr(oCurrent.oFile.type.indexOf("/") + 1, 4);
                    }
                }
            }
            oCurrent.oFile.name = oCurrent.oFile.name || "Unknown";
            oCurrent.oFile.size =  oCurrent.oFile.size || 0;
            oCurrent.oFile.type = oCurrent.oFile.type || "Unknown";


            aRows.push([ i, oCurrent.oFile.name, oCurrent.oFile.size, oCurrent.oFile.type ]);
            
            oOpts.iTotal += oCurrent.oFile.size;
        }
        ...
        oWO.serverAction("DoStartUpload", [ ],  df.sys.vt.serialize(aRows), function(oEv){
...
df.uploadFilesProgressDialog = function(oOpts){
    ...
    oWO = oOpts.oWO;
    sFilesTrans = oWO.getWebApp().getTrans("files");
    sFileTrans = oWO.getWebApp().getTrans("file");
