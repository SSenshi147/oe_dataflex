...
openHtml : function(aHtml){
    ...
    //  Generate body HTML
    aHtml.push('<div class="', this._sCardClass, '_Body ', (this.pbShowBorder ? 'WebCC_BodyBorder' : 'WebCC_BodyNoBorder') , (this.pbShowCaption ? ' WebCC_HasCaption' : '') ,'">');
},
...
touchSwipe : function(bPointer, oEv){
    ...
    function touchEnd(oEv){
        ...
        if(bPointer){
            df.dom.off("pointerup", window, touchEnd);
            df.dom.off("pointercancel", window, touchEnd);
            df.dom.off("pointermove", window, touchMove);
        }else{
            df.dom.off("touchend", window, touchEnd);
            df.dom.off("touchcancel", window, touchEnd);
            df.dom.off("touchmove", window, touchMove);
        }
